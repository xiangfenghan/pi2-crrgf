...
class VueEnchere
{
	...
	public static function afficherUneEnchere($oEnchere)
	{
		...
		echo "<article class='col-md-7 col-sm-6'>".
			 "<form action='#'><input type='hidden' id='idEnchere' value='".$oEnchere->getIdEnchere()."'>".
				...
				"<div class='form-group'>".
					"<a href='index.php?page=listeOffre&idEnchere=".$oEnchere->getIdEnchere()."'>Offres</a>".
			 ...
			 "</form></article>";
			 // VueCommentaire::afficherFormPoserUnCommentaires($_SESSION['idUser']);
			$oCommentaire = new Commentaire(0," "," ","Non",0,$oEnchere->getIdEnchere());
			$oCommentaires = $oCommentaire->recherListeCommentairesParIdEnchere($oEnchere->getIdEnchere());
			VueCommentaire::afficherListeCommentaires($oCommentaires);
			var_dump($oCommentaire->recherUnCommentParIdEnchere());
		Vue::footer();
	}
	...
	public static function formModEnchere($oEnchere)
	{
		...
		echo "
		<article class=\"col-md-7 col-sm-6\">
			<form action=\"index.php?page=gestionEnchere&action=mod&idEnchere=".$oEnchere->getIdEnchere(); echo "\" method=\"post\" class=\"form-horizontal\">
				...
				<div class=\"form-group\">
					...
					<input type=\"text\" name=\"prixDebut\" value=\"".$oEnchere->getPrixDebut(); echo "\"> \$CAD | Montant debut
					<input type=\"text\" name=\"prixAug\" value=\"".$oEnchere->getMontantAugment(); echo "\"> \$CAD | Montant incrementation
				</div>
				<div class=\"form-group\">
					<label>Prix \"Acheter Maintenant\"</label>
					<input type=\"text\" name=\"prixDirecte\" value=\"".$oEnchere->getPrixAcheterMaintenant(); echo "\"> \$CAD
				</div>
				<div class=\"form-group\">
					<label>Duree</label>
					<select name=\"duree\">
